...
def t_waves(signal):
    if len(signal) == 0:
        raise ValueError('Signal should not be empty')

    if type(signal) != np.ndarray:
        raise TypeError('Signal should be a numpy ndarray')
...
def test_find_total():
    assert find_total([-1, 0, 1]) == 0
    assert find_total([1, 2, 3]) == 6
...
def test_twave_output():
    t_x, t_y = t_waves(signal_5)
    assert type(t_x) == np.ndarray
    assert type(t_y) == np.ndarray
    assert len(t_x) != 0
    assert len(t_y) != 0